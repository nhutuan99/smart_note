...
function matchWalletByCasso(wallets, bankName, bankAbbr) {
...
__name(matchWalletByCasso, "matchWalletByCasso");
function parseCassoDate(when) {
  try {
...
__name(parseCassoDate, "parseCassoDate");
async function handleCassoWebhook(request, env) {
  const secret = request.headers.get("secure-token") || request.headers.get("Authorization")?.replace("Bearer ", "") || "";
  ...
  for (const record of records) {
    ...
    const bankName = record.bankName ?? record.bank_name ?? "";
    const bankAbbr = record.bankAbbr ?? record.bank_abbr ?? "";
    const description = record.description ?? record.memo ?? "";
    const when = record.when ?? record.createdAt ?? "";
    const walletId = matchWalletByCasso(wallets, bankName, bankAbbr);
    const cassoTid = String(record.tid ?? record.id ?? "");
    ...
    notiList.unshift({
      ...
      title: `[Casso] ${type === "income" ? "Ti\u1EC1n v\xE0o t\xE0i kho\u1EA3n" : "Ti\u1EC1n ra t\xE0i kho\u1EA3n"}`,
      body: `${type === "income" ? "+" : "-"}${amount.toLocaleString("vi-VN")}\u0111 \u2022 Ghi v\xE0o v\xED: ${walletName}`,
      read: false,
      createdAt: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    await putJSON(env.SMART_NOTE_KV, `users/${userId}/notifications`, notiList);
    try {
      const pushTitle = type === "income" ? "\u{1F4B0} Ti\u1EC1n v\xE0o t\xE0i kho\u1EA3n" : "\u{1F4B8} Ti\u1EC1n ra t\xE0i kho\u1EA3n";
  ...
  return jsonResponse({
    success: true,
    message: `\u0110\xE3 ghi ${created.length} giao d\u1ECBch t\u1EEB ng\xE2n h\xE0ng`,
    data: created
  });
}
__name(handleCassoWebhook, "handleCassoWebhook");
